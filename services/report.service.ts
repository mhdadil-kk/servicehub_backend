...
export class ReportService implements IReportService {
  ...
  async createReport(reporterId: string, data: CreateReportInput): Promise<IReport> {
    ...
    const report = await this._reportRepository.create({
      ...
      reportedId: data.reportedId as any,
      bookingId: data.bookingId ? (data.bookingId as any) : undefined,
  ...
  async takeAction(
    ...
    const reporterUserId = (report.reporterId as any)?._id 
      ? (report.reporterId as any)._id.toString() 
      : report.reporterId.toString();
      
    const reportedUserId = (report.reportedId as any)?._id 
      ? (report.reportedId as any)._id.toString() 
    ...
    if (action === "reject") {
      ...
      await userModel.findByIdAndUpdate(reportedUserId, { status: "blocked" }).exec();
